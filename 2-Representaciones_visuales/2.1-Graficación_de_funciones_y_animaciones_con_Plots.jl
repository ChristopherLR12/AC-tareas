...
#El apóstrofe transpone la matriz  (-2:0.2:2), pues un vector rengló es equivalente a una matriz de un rengló.
...
# ╔═╡ ffcdbb3f-aa14-418c-8919-5828289b0a82
begin

H = 3 #Altura inicial
θ = π/4 #Ángulo de lanzamiento
v_0 = 10 #Velocidad de lanzamiento inicial
...
@gif for t in (0:0.025:T)
	plot(w,v_0x*(0:0.01:t), xaxis=[0,x_m], yaxis=[0,y_m], style = [:dot], color = "white", lw = [2], legend = false, title=("Tiro parabólico"), xlabel=L"x", ylabel=L"y", background="black")
	scatter!(z,v_0x*t,v_0x*t, xaxis=[0,x_m], yaxis=[0,y_m], color = "yellow", ms = [6], legend = false)
    end
end

# ╔═╡ a581ca44-b2f5-4aae-93bc-5776bdf9631c


# ╔═╡ 993eae38-baff-4102-a47d-1893cacfa3f2
...
@gif for t in (0:0.025:T_v)
	plot(w_1,r_x*(0:0.01:t), xaxis=[0,x_M], yaxis=[0,y_M], style = [:dot], color = "yellow", lw = [2], legend = false, title=("Tiro parabólico"), xlabel=L"x", ylabel=L"y", background="black")
	scatter!(z_1,r_x*t,r_x*t, xaxis=[0,x_M], yaxis=[0,y_M], color = "white", ms = [6], legend = false)
    end
end
end

# ╔═╡ a891b060-ca42-49b0-b262-eb7b0163f511
begin
#Prueba de función
	p(4,6,π/5,0)
end
...
# ╔═╡ cd9e8b26-7f3f-499b-8d82-7bd445491be8
begin
...
@gif for n in (0:0.1:3)
	surface((0:0.1:2π),(0:0.1:2*π*n),f,yaxis=[0,y_max],zaxis=[-2,2],background="black", color=:davos)
    end
