...
# ╔═╡ 87305bd6-0182-49bd-bf74-5a80013188b3
using Plots, PlutoUI

# ╔═╡ 84005d8b-d8c8-4909-a303-ebc6d7da1791
html"""
<style>
	main {
		margin: 0 auto;
...
function newton(f,f′,x0,n)
	for i in 1:n
		x0=x0-(f(x0)/f′(x0)) #Iteraciones siguientes
	end
	return x0
end
...
function newtonRecursiva(f,f′,x0,n,c) #Se agregó un parámetro para poder aplicar un contador
	
	x0=x0-(f(x0)/f′(x0))
	c=c+1 #Contador
	if c<n
	newtonRecursiva(f,f′,x0,n,c) #Recursividad
	else
		return x0
	end
end
...
begin
	...
	if ε>0 #Condición ε
		x0=x0-(f(x0)/f′(x0)) #Nuevo valor de x0
        if abs((f(x0)/f′(x0)))>ε #Distancia
        newtonDistancia(f,f′,x0,ε) #Recursividad
        else
            return x0
        end
    else
        return "ERROR: El valor de ε es menor o igual a cero." #Error
    end
end

#Prueba de función
newtonDistancia(f,f′,2,0.1)
end
...
function newtonDN(f,x0,h,n)
	for i in 1:n
		x0=x0-(f(x0)/derivadaNumérica(f,x0,h)) #Iteraciones siguientes
	end
	return x0
end
...
f_1(x) = x^2-25 # Luego, usa esta celda para reasignar `f_1`
...
@bind l1 Slider(-10:0.1:10, default=1) #x0

# ╔═╡ 5b350ae1-570d-4c2e-a548-45e0cce648db
@bind l2 Slider(1000:1000:1000000, default=1000) #h

# ╔═╡ d14a3f87-ef97-4de1-84b6-17e4794bea9e
@bind l3 Slider(1:1000, default=100) #n

# ╔═╡ f90c96cf-0de4-47ac-a659-f5e05b876e62
begin
	function s(n) #Valor de función respecto a raíz
	A=Vector{Tuple{Float64,Float64}}(undef, n)
	    for i in 1:n
	    r=newtonDN(f_1,l1,l2,i)
	    A[i]=(r,f_1(r))
	end
	return plot(A,legend=false, marker=true, color=:gray, title="Valor de función respecto a raíz",xaxis="Raíz", yaxis="f(raíz)")
	end
	
	s(l3)
end

# ╔═╡ 1a6e6357-34e3-4fa4-8e89-91ca2a3e2c21
md"""
***Se agregó gráfica de la raíz obtenida respecto al número de iteración***:
"""

# ╔═╡ b6f27148-fce5-474b-9232-6985ac584cb1
begin
	function q(n) #Raíz respecto a número de iteración
	A=Vector{Tuple{Float64,Float64}}(undef, n)
	    for i in 1:n
	    r=newtonDN(f_1,l1,l2,i)
	    A[i]=(i,r)
	end
	return plot(A,legend=false, marker=true, color=:gray, title="Raíz respecto a número de iteración",xaxis="Número de iteración", yaxis="Raíz")
	end
